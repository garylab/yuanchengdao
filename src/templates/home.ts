...
export function homePage(jobs: Job[], countries: CountryFilter[], locations: LocationFilter[], page: number, hasMore: boolean, opts: HomePageOptions = {}): string {
  ...
  const quickEntries = showDiscovery ? `
    <div class="max-w-5xl mx-auto mt-3 flex flex-wrap gap-2 text-xs">
      ...
      <a href="/salary-report" class="px-3 py-1.5 rounded-full bg-white border border-surface-200 text-surface-700 hover:border-brand-300 hover:text-brand-600 transition no-underline">💰 薪资报告</a>
  ...
  if (showDiscovery && topSalaryJobs.length > 0) {
    discoveryCols.push(`
      <div class="bg-white rounded border border-surface-200 p-4">
        <div class="flex items-center justify-between mb-2"><h3 class="text-sm font-bold text-surface-900">本周高薪</h3><a href="/salary-report" class="text-xs text-surface-400 hover:text-brand-500 no-underline">薪资报告 →</a></div>
