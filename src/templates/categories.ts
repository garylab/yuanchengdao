  ...
  const content = `
    ...
    <div class="max-w-5xl mx-auto px-4 mt-4">
      <div class="flex items-center justify-between mb-4">
        <h1 class="text-xl font-bold text-surface-900">职位</h1>
        <form action="/categories" method="GET" class="relative w-48">
          <input type="text" name="q" value="${query ? escapeHtml(query) : ''}" class="w-full px-3 py-1.5 pr-8 rounded border border-surface-200 text-sm outline-none focus:ring-1 focus:ring-brand-300 focus:border-brand-300 placeholder:text-surface-400" placeholder="搜索职位...">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        ${terms.map(t => `
          <a href="/category/${escapeHtml(t.slug)}" class="block bg-white rounded border border-surface-200 p-5 hover:border-brand-300 hover:shadow-sm transition no-underline group">
      ...
      ${terms.length === 0 ? `
        <div class="text-center py-20 text-surface-400">
          <p class="text-lg">${query ? '没有匹配的职位' : '暂无职位'}</p>
        </div>
      ` : ''}
    </div>`;

  const pageTitle = '职位 - 远程岛';
